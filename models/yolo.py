...
class Segment_Decoupled(Detect_Decoupled):
    ...
    def __init__(self, nc=80, anchors=(), nm=32, npr=256, ch=(), inplace=True):
        ...
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect_decoupled = Detect_Decoupled.forward

    def forward(self, x):
        p = self.proto(x[0])
        x = self.detect_decoupled(self, x)
        return (x, p) if self.training else (x[0], p) if self.export else (x[0], p, x[1])
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_Decoupled, Segment, Segment_Decoupled)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_Decoupled, Segment, Segment_Decoupled)):
            ...
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_Decoupled)) else self.forward(x)
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        if m is Detect or m is Segment:
            for mi, s in zip(m.m, m.stride):  # from
                b = mi.bias.view(m.na, -1)  # conv.bias(255) to (3,85)
                b.data[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
                b.data[:, 5:5 + m.nc] += math.log(0.6 / (m.nc - 0.99999)) if cf is None else torch.log(
                    cf / cf.sum())  # cls
                mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
        elif m in {Detect_Decoupled, Segment_Decoupled}:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Detect_Decoupled, Segment, Segment_Decoupled}:
            # 添加输入通道的维度参数
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
            if m in {Segment, Segment_Decoupled}:
